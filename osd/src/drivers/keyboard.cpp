...
// it would be great to have internal enumeration for different keys
...
dataport(0x60), // initializing the value of the data port and the command port
commandport(0x64)
{
    this->handler = handler;
}

KeyboardDriver :: ~KeyboardDriver() // destructor for the driver class
{}

void printf(char *);
void printfHex(uint8_t);

// this is reading and writing the values in the commandport and dataport and activating the keyboard
void KeyboardDriver :: Activate()
{
    while(commandport.Read() & 0x1)
        dataport.Read(); // reading from dataport
...
// this is one of the main function for handling the keyboard request
uint32_t KeyboardDriver::HandleInterrupt(uint32_t esp)
{
    ...
    // also taken the case of shift when it is clicked
    static bool Shift = false;

        switch(key)
        {
            // we have to write for all the alphabets 
            ...
            // we can ignore many like that
            case 0x45: case 0xC5: break; // this is for num lock so ignored

            // here we have written for when key is left from pressing then nothing should happen
            // when key is pressed two interrupts are called one for press and one for releasing
            default:
                if(key < 0x80)
                {
                    printf("KEYBOARD 0x");
                    printfHex(key);
                    // print statement when interrupt is called from keyboard
                }
                break;
        }
